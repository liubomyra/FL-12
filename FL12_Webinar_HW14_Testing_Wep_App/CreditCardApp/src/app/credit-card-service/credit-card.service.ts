...
export class CreditCardService {
  ...
  public testCreditCard(cardNumber: string, cardName: string) {
    ...
    if (
      !this.isModuleTenDigitsValid(cardNumber) ||
      !this.isCardNumberPrefixValid(cardNumber, cardTypeIndex)
    ) {
  ...
  private isCardNumberLengthValid(
    cardNumber: string,
  ...
  private isCardNumberPrefixValid(
    cardNumber: string,
    cardTypeIndex: number
  ): boolean {
    const cardNo: string = this.cutSpaces(cardNumber);
